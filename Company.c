...
char* printCompanyName(char* base, ...)
{
		char* str = _strdup(base);
		if (str == NULL)
			return NULL;

		va_list strings;
		char* temp = NULL, *currentStr;

		va_start(strings, base);
		currentStr = va_arg(strings, char*);
		while (currentStr != NULL)
		{
			temp = (char*)realloc(str, strlen(str) + strlen(currentStr) + 2);
			if (temp != NULL)
				str = temp;
			strcat(str, "_");
...
{
#ifdef DETAIL_PRINT
	printf("Company %s:\n", printCompanyName(pComp->name, "Hachi", "Babit", "Ba", "Olam", NULL));
